...
Object.assign(paths,{play:'M8 5l12 7-12 7Z',pause:'M8 5v14 M16 5v14',refresh:'M20 7a9 9 0 1 0 1 8 M20 2v6h-6',live:'M12 8v8 M8 12h8 M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0',fit:'M8 3H3v5 M16 3h5v5 M3 16v5h5 M21 16v5h-5 M8 12h8 M12 8v8',back:'M9 5l-6 6 6 6 M3 11h12a6 6 0 0 1 6 6',close:'M6 6l12 12 M18 6L6 18',chevron:'M9 5l7 7-7 7',check:'M5 12l4 4L19 6'});
...
let modalOrigin=null, modalRevision=0, modalDirty=false;
function toast(text){
...
function modal(html,options={}){
 ...
 const eyebrow=template.content.querySelector('.step-label');
 ...
 const head=document.createElement('div');head.className='modal-header';const titles=document.createElement('div');if(eyebrow)titles.append(eyebrow);titles.append(heading);head.append(titles);
 ...
 content.replaceChildren(head,body,footer);dialog.dataset.family=options.family||(body.querySelector('.editor')?'editor':body.querySelector('.markdown-reader')?'reader':'standard');
...
function closeModal(force=false){
 if(modalDirty&&!force){toast('Há alterações no editor. Use “Descartar alterações” ou revise as diferenças.');return}
 $('#modal').close();
}
...
function render(){
 ...
 $('#codex-status').textContent=state.events.some(e=>e.source==='codex-hook')?'Hooks recebidos · cobertura parcial':'Atividade desconhecida';
 $('#gbrain-status').textContent=state.config.gbrainWorkspace?'Fonte selecionada':'Consulte a conexão nos ajustes';
 renderPlayback();
}
function renderTree(){
...
function renderAtlas(){
 if(!atlasController){atlasController=new OracleAtlas($('#atlas'),{onSelect:(id,leaf)=>{selected=id;selectedSkill=leaf;renderInspector()},onOpen:safe(openNote),onLayout:safe(async layout=>{if(replay)return;await call('saveLayout',{layout});state.config.layout=structuredClone(layout)}),onZoom:value=>{$('#zoom-label').textContent=Math.round(value*100)+'%'}})}
 atlasController.update({collections:replaySession?.kind==='formation'&&replay?OracleReplay.formationAt(replaySession.entries,state.collections,cursor).collections:state.collections,entries:replaySession?.kind==='formation'&&replay?OracleReplay.formationAt(replaySession.entries,state.collections,cursor).entries:visibleEntries(),selected,selectedLeaf:selectedSkill,detail:Number($('#density').value),events:replay?timelineEvents().slice(0,cursor+1):state.events,replay,reduced:$('#motion').checked,economy:$('#economy').checked,layout:state.config.layout,hidden:view!=='map'||window.oracleWindowVisible===false,paused:visualPaused||$('#modal').open});
}
...
async function openNote(path){const revision=modalRevision;const document=await call('read',{path});if(revision!==modalRevision)return;readDocument=document;readDocument.relative=path;modal(`<span class="step-label">FONTE CANÔNICA · MARKDOWN</span><h1>${esc(path.split('/').slice(-2).join('/'))}</h1><div class="source">${esc(readDocument.path)}<br>SHA-256 ${readDocument.hash}</div><article class="markdown-reader">${markdown(readDocument.text)}</article>${actions(`<button class="secondary" id="reveal-note">Mostrar no Finder</button>${path.endsWith('SKILL.md')?'<button class="primary" id="edit-note">Criar versão pessoal</button>':''}`)}`);bindMarkdown($('#modal-content'));$('#reveal-note').onclick=safe(()=>call('reveal',{path}));$('#edit-note')?.addEventListener('click',()=>editNote())}
function editNote(draft=readDocument.text){modal(`<h1>Versão pessoal</h1><p>O arquivo do fornecedor será preservado. Revise o diff antes de salvar. O Codex não recarrega esta versão automaticamente.</p><div class="source">${esc(readDocument.relative)} · base ${readDocument.hash.slice(0,12)}</div><textarea class="editor" id="editor" aria-label="Conteúdo SKILL.md" autocorrect="off" autocapitalize="off" spellcheck="false" writingsuggestions="false">${esc(draft)}</textarea>${actions('<button class="secondary" id="discard-edit">Descartar alterações</button><button class="primary" id="diff">Revisar diferenças</button>')}`);modalDirty=draft!==readDocument.text;$('#editor').oninput=()=>modalDirty=$('#editor').value!==readDocument.text;$('#discard-edit').onclick=()=>closeModal(true);$('#diff').onclick=()=>{const value=$('#editor').value;const old=readDocument.text.split('\n'),now=value.split('\n');let lines='';for(let i=0;i<Math.max(old.length,now.length);i++){if(old[i]===now[i])lines+='  '+esc(old[i]??'')+'\n';else{if(old[i]!==undefined)lines+=`<span class="diff-remove">− ${esc(old[i])}</span>\n`;if(now[i]!==undefined)lines+=`<span class="diff-add">+ ${esc(now[i])}</span>\n`}}modal(`<h1>Revisar alteração</h1><p>Uma versão separada será criada com procedência. Se a fonte mudou, sua proposta será preservada como conflito.</p><pre>${lines}</pre>${actions('<button class="secondary" id="discard-diff">Descartar alterações</button><button class="secondary" id="back-editor">Voltar ao editor</button><button class="primary" id="save-version">Salvar versão pessoal</button>')}`,{family:'editor'});modalDirty=value!==readDocument.text;$('#discard-diff').onclick=()=>closeModal(true);$('#back-editor').onclick=()=>editNote(value);$('#save-version').onclick=safe(async()=>{const r=await call('saveVersion',{path:readDocument.relative,hash:readDocument.hash,text:value});closeModal(true);toast(r.status);await refresh()})}}
...
function settings(){
 ...
 modal(`<span class="step-label">PREFERÊNCIAS</span><h1>Ajustes do Oracle</h1>
 <p class="settings-intro">Um espaço seu, conectado às fontes que você escolheu.</p>
 <section class="settings-section"><h2>Conhecimento e memória</h2><div class="setting-group">
 ${row('restart-setup','Configurar Oracle',state.config.vault?'Gerenciar a configuração do seu universo':'Conectar uma pasta e começar')}
 ${row('gbrain-connect','Conectar Second Brain','Selecionar uma instalação GBrain existente')}
 ${row('gbrain-memory','Consultar memória','Explorar fontes e relações verificáveis')}
 ${row('gbrain-review','Revisar entrevista','Conferir suas respostas antes de confirmar')}
 </div></section>
 <section class="settings-section"><h2>Integração e privacidade</h2><div class="setting-group">
 ${row('bridge-review','Ponte com Codex','Workspace, hooks e cobertura observada')}
 ${row('protect-settings','Proteção de acesso',state.config.protected?'Proteção ativada':'Ativar Touch ID ou senha do macOS','lock')}
 ${row('revoke','Revogar fontes','Interromper o acesso do Oracle às pastas autorizadas')}
 </div></section>
 <section class="settings-section"><h2>Aplicativo</h2><div class="setting-group">
 ${row('updates-settings','Atualizações','Versões, fontes e recuperação','refresh')}
 ${row('graphics-diagnostics','Diagnóstico do atlas','Renderização, desempenho e origem dos dados')}
 ${row('export-view','Exportar imagem','Salvar uma captura desta visão')}
 </div></section>
 <details class="source-details"><summary>Fontes e estado desta instalação</summary><div class="source">${window.ORACLE_PREVIEW||state.config.fixture?'Ambiente de validação · dados sintéticos':'Fonte local selecionada pelo usuário'}<br>Vault: ${esc(state.config.vault||'Não selecionado')}<br>Motor GBrain: ${esc(state.config.gbrainWorkspace||'Perfil próprio do Oracle, se instalado')}<br>Codex: ${state.events.filter(e=>e.source==='codex-hook').length} recibos · cobertura parcial<br>Oracle 0.2.0 · desenvolvimento, assinatura ad hoc</div></details>${actions()}`,{family:'settings'});
 $('#restart-setup').onclick=()=>state.setup?.confirmed?setupContinuation():showSetup(0);
 $('#gbrain-memory').onclick=safe(memory);$('#gbrain-review').onclick=safe(reviewGBrain);$('#bridge-review').onclick=safe(reviewBridge);
 $('#graphics-diagnostics').onclick=graphicsDiagnostics;$('#updates-settings').onclick=safe(()=>showUpdates(false));
 $('#export-view').onclick=safe(async()=>{closeModal();const path=await call('exportSnapshot');if(path)toast('Imagem salva: '+path)});
 $('#gbrain-connect').onclick=safe(async()=>{await call('chooseGBrain');await refresh();settings()});
 $('#protect-settings').onclick=safe(async()=>{await call('protect');await refresh();settings();toast('Proteção ativada')});
 $('#revoke').onclick=safe(async()=>{await call('revoke');live();selected=null;selectedSkill=null;query='';await refresh();settings()});
}
function renderPlayback(){
 const max=replaySession?.kind==='journal'?Math.max(0,replaySession.events.length-1):state.collections.length*2;
 $('#timeline').max=max;$('#timeline').value=replay?cursor:max;
 const phase=cursor===0?'Núcleo':cursor<=state.collections.length?'Especialistas':'Skills';
 $('#replay-label').textContent=replay?(replaySession?.kind==='journal'?'Journal':phase):'Ao vivo';
 $('#replay-label').classList.toggle('is-replay',replay);
 $('#play').innerHTML=icon(timer?'pause':'play');
 const label=timer?'Pausar timelapse':replay?'Retomar timelapse':'Iniciar timelapse';
 $('#play').setAttribute('aria-label',label);$('#play').dataset.tooltip=label+' · apenas visual';if(tooltipTarget===$('#play'))$('#tooltip').textContent=$('#play').dataset.tooltip;
 $('#timeline').setAttribute('aria-valuetext',replay?`${phase}, etapa ${cursor} de ${max}`:'Visão atual');
 $('#live').disabled=!replay;$('#speed').setAttribute('aria-label',`Velocidade do timelapse: ${speed} vezes`);
 $('#reset-layout').disabled=replay;
}
function live(){
...
async function ensureReplay(){
 ...
 replaySession={kind:'formation',entries:structuredClone(state.entries)};cursor=0;selected=null;selectedSkill=null;renderInspector();
 // Freeze the visual snapshot only. No installation or filesystem command is invoked.
}
function projectReplay(){
 replayProjection=replaySession.kind==='journal'?OracleReplay.projectJournal(replaySession.baseline,replaySession.events,cursor):null;
 renderProgress();renderAtlas();renderResults();
}
async function scrub(){const value=Number($('#timeline').value);clearInterval(timer);timer=null;await ensureReplay();replay=true;cursor=value;projectReplay()}
async function play(){
 if(timer){clearInterval(timer);timer=null;renderPlayback();return}
 await ensureReplay();replay=true;
 const max=replaySession.kind==='journal'?replaySession.events.length-1:state.collections.length*2;
 if(cursor>=max)cursor=0;projectReplay();
 timer=setInterval(()=>{cursor=Math.min(cursor+1,max);if(cursor===max){clearInterval(timer);timer=null}projectReplay()},800/speed);renderPlayback();
}
async function startJournal(){
 const data=await call('replayData');if(!data.events?.length)throw Error('Nenhum journal de instalação para reproduzir.');
...
$('#tools').onclick=plugin;$('#gmail').onclick=plugin;$('#activity').onclick=activity;$('#conversations').onclick=safe(conversations);$('#instructions').onclick=safe(instructions);
...
$('#speed').onclick=()=>{speed=speed===4?1:speed*2;$('#speed').textContent=speed+'×';if(timer){clearInterval(timer);timer=null;play()}renderPlayback()};
$('.wordmark').onclick=e=>{e.preventDefault();setView('map');atlasController?.fit()};
$('#updates').onclick=safe(()=>showUpdates(true));
...
call('boot').then(async b=>{if(b.locked)window.oracleLock();else{await refresh();$('#app').inert=false}}).catch(e=>{if($('#lock-screen').hidden)$('#app').inert=false;toast(e.message)});
...
async function reviewBridge(){const revision=modalRevision;const r=await call('bridgeReceipt');if(revision!==modalRevision)return;modal(`<h1>Ponte com Codex</h1><p>${r.workspace?'Abra este workspace no Codex e revise os hooks pelo mecanismo oficial.':'A ponte ainda não foi preparada. Continue pelo pedido de configuração.'}</p>${r.workspace?`<div class="source">${esc(r.workspace)}</div><span class="pill">Preparada · confiança não verificada</span><p>A memória usa o perfil isolado do Oracle. A observação cobre tarefas que utilizam este workspace e seus hooks confiados.</p>`:''}<p>${state.events.filter(e=>e.source==='codex-hook').length} recibos de hooks encontrados. Receber um evento não comprova cobertura de todas as ferramentas.</p>${actions('<button class="primary" id="bridge-codex">Abrir Codex</button>')}`);$('#bridge-codex').onclick=safe(()=>call('openCodex'))}
...
const updateNames={gbrain:'Second Brain',cognee:'Cognee',skills:'Skills'};
const updateStates={not_checked:'Não verificado',configured:'Fonte configurada',current:'Em dia',updated:'Atualizado',external:'Instalação externa',compatibility_required:'Compatibilidade pendente',not_adopted:'Não adotado',not_configured:'Fonte não configurada',error:'Requer atenção',preserved_edits:'Edições preservadas',rolled_back:'Recuperado'};
async function showUpdates(start=false){
 if(start){await call('updateStart');updateBusy=true;$('#updates').classList.add('busy')}
 modal(`<span class="step-label">MANUTENÇÃO DO UNIVERSO</span><h1>Atualizações</h1>
 <p>Versões compatíveis, com verificação de integridade e recuperação.</p>
 <div class="update-status" role="status" aria-live="polite"><span id="update-message">Consultando fontes…</span><progress id="update-progress" aria-label="Progresso das atualizações"></progress></div>
 <div id="update-results" class="update-results"></div>
 <details class="source-details"><summary>Repositório central de skills</summary><p>Informe o repositório quando estiver disponível. Ele precisa publicar um release com o manifesto Oracle.</p><label class="field">Repositório GitHub<input id="skills-repository" type="url" placeholder="https://github.com/organização/repositório" autocomplete="off" spellcheck="false"></label><button class="secondary" id="save-skills-source">Salvar fonte</button></details>
 <details class="source-details"><summary>Fontes e limites</summary><div class="source">GBrain: github.com/garrytan/gbrain<br>Cognee: github.com/topoteretes/cognee · adoção pendente<br>O botão verifica e aplica versões autorizadas neste clique. A instalação pessoal externa é preservada. Novas versões do motor precisam de validação do adaptador e do banco; nenhuma migração é executada pelo atualizador.</div><div id="catalog-origins"></div></details>
 <div id="update-recovery" class="recovery-actions"></div>
 ${actions('<button class="primary" id="check-updates">Verificar e atualizar</button>')}`,{family:'updates'});
 const revision=modalRevision;
 $('#save-skills-source').onclick=safe(async()=>{await call('configureSkillSource',{repository:$('#skills-repository').value});toast('Fonte salva. Verifique as atualizações para consultar o catálogo.');});
 $('#check-updates').onclick=safe(()=>showUpdates(true));
 let first=true;
 const poll=async()=>{
  try {
   const status=await call('updateStatus');
   updateBusy=!!status.busy;$('#updates').classList.toggle('busy',updateBusy);
   if($('#modal').open&&modalRevision===revision){
    $('#update-message').textContent=status.message||'Pronto para verificar';
    ...
    $('#check-updates').disabled=updateBusy;$('#save-skills-source').disabled=updateBusy;
    if(first){$('#skills-repository').value=status.skills_repository||'';first=false}
    const results=status.results?.length?status.results:[{id:'gbrain',status:'not_checked',message:'Versão integrada '+status.gbrain_version},{id:'cognee',status:'not_adopted',message:'Avaliação disponível no projeto.'},{id:'skills',status:status.skills_repository?'configured':'not_configured',message:status.skills_repository||'Aguardando o futuro repositório central.'}];
    $('#update-results').innerHTML=results.map(r=>`<section class="update-result"><div>${icon(r.status==='error'?'tool':r.status==='current'||r.status==='updated'?'check':r.id==='skills'?'folder':'brain')}<h2>${updateNames[r.id]||esc(r.id)}</h2><span class="pill ${r.status==='error'?'warning':''}">${updateStates[r.status]||esc(r.status)}</span></div><p>${esc(r.message)}</p>${r.version?`<small>Versão ${esc(r.version)}</small>`:''}</section>`).join('');
    $('#update-recovery').innerHTML=(status.gbrain_rollback?'<button class="secondary" data-rollback="rollback-gbrain">Restaurar motor anterior</button>':'')+(status.skills_rollback?'<button class="secondary" data-rollback="rollback-skills">Restaurar skills anteriores</button>':'');
    $$('[data-rollback]').forEach(b=>{b.disabled=updateBusy;b.onclick=safe(async()=>{await call('updateStart',{operation:b.dataset.rollback});showUpdates(false)})});
    $('#catalog-origins').innerHTML=(status.catalog_origins||[]).map(c=>`<div class="source">${esc(c.id)} · ${esc(c.repo)}<br>Commit ${esc(c.commit)}</div>`).join('');
   }
