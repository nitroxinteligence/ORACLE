...
def main():
    ...
    for name in ('engine', 'state/gbrain/profile', 'workspace', 'home', 'tmp', 'state/events', 'calls', 'other-source', 'vault/INBOX/oracle-memory'):
    ...
    env = {'PATH': '/usr/bin:/bin:/usr/sbin:/sbin', 'HOME': str(fixture / 'home'),
           'TMPDIR': str(fixture / 'tmp'), 'LANG': 'en_US.UTF-8', 'GBRAIN_HOME': str(fixture / 'state/gbrain/profile'),
           ...
           'BUN_RUNTIME_TRANSPILER_CACHE_PATH': str(fixture / 'bun-cache'), 'ORACLE_RECEIPT_DIR': str(fixture / 'state/events')}
