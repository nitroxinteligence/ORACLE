...
async function refresh(){
 ...
 const task=(async()=>{const next=await call('snapshot');if(sequence!==refreshSequence||!$('#lock-screen').hidden)return;
  ...
  if(!updateBusy)call('updateStatus').then(reflectUpdateStatus).catch(()=>{});if(state.scanError&&scanErrorChanged)toast(state.scanError);
...
function renderTree(){
 ...
 const departments=OracleDepartments.inventory(state.collections,visibleEntries(),state.config.departmentAssignments);
 html+='<div class="tree-section-label">Departamentos</div>'+departments.map(d=>`<details ${opened.has(d.id)||d.specialists.some(s=>s.id===selected)?'open':''}><summary data-department="${esc(d.id)}">${icon(d.icon)}${esc(d.name)}<span class="count">${d.specialists.length}</span></summary><div>${d.specialists.map(c=>`<details ${selected===c.id||opened.has(c.id)?'open':''}><summary data-collection="${esc(c.id)}">${icon(c.icon||'tool')}${esc(c.name)}<span class="count">${c.skills.length}</span></summary><div>${c.skills.slice(0,9).map(e=>`<button class="leaf" data-path="${esc(e.path)}">${icon('note')}${esc(title(e).replace(/-/g,' '))}</button>`).join('')}${c.skills.length>9?`<button data-prefix="SISTEMA/skills/${esc(c.id)}">Ver todas →</button>`:''}${!c.skills.length?'<small class="empty-collection">Nenhuma skill nesta fonte</small>':''}</div></details>`).join('')}</div></details>`).join('');
...
$('#tree').addEventListener('click',event=>{const department=event.target.closest('[data-department]');if(department)atlasController?.navigateDepartment(department.dataset.department);});
...
function renderInspector(){
 ...
 if(atlasController?.department){const d=OracleDepartments.inventory(state.collections,visibleEntries(),state.config.departmentAssignments).find(d=>d.id===atlasController.department);if(d){$('#inspector-title').textContent=d.name;$('#inspector').innerHTML=`<span class="pill">Departamento</span><p>${d.specialists.length} especialistas · ${d.skills.length} skills</p><p class="muted">Escolha um especialista para explorar o catálogo completo. O mapa não move os arquivos do vault.</p>`;return;}}
...
function departmentSettings(){
 const catalog=OracleDepartments.inventory(state.collections,visibleEntries(),state.config.departmentAssignments);
 const current=new Map(catalog.flatMap(d=>d.specialists.map(s=>[s.id,d.id])));
 modal(`<h1>Organizar departamentos</h1><p>Atribuição visual explícita. Não renomeia, move ou altera suas skills. Especialistas sem atribuição ficam em “Sem departamento”.</p><div class="department-settings">${state.collections.map(c=>`<label class="department-setting"><span>${esc(c.name)}</span><select data-department-choice="${esc(c.id)}">${OracleDepartments.definitions.map(d=>`<option value="${d.id}" ${current.get(c.id)===d.id?'selected':''}>${esc(d.name)}</option>`).join('')}</select></label>`).join('')}</div>${actions('<button class="primary" id="save-departments">Salvar organização</button>')}`);
 $('#save-departments').onclick=safe(async()=>{const assignments=Object.fromEntries($$('[data-department-choice]').map(e=>[e.dataset.departmentChoice,e.value]));await call('saveDepartments',{assignments});state.config.departmentAssignments=assignments;if(atlasController)atlasController.topologyKey=null;await refresh();toast('Organização salva. Os arquivos permanecem no lugar.');});
}
...
function openSearch(initial=''){
 ...
 <label class="search search-field">${icon('search')}<input id="universe-query" type="search" autocomplete="off" spellcheck="false" aria-label="Buscar notas e skills" placeholder="Nome ou caminho…" value="${esc(prefix?'':initial)}" aria-controls="search-results"></label>
 ...
 const search=()=>{
  ...
  const all=state.entries.filter(e=>!e.directory&&(!prefix||e.path===prefix||e.path.startsWith(prefix+'/'))&&terms.every(t=>(title(e)+' '+e.path).toLocaleLowerCase('pt-BR').includes(t)));
 ...
 const poll=async()=>{
  try{
   ...
   if($('#modal').open&&modalRevision===revision){
    ...
    const results=(status.results?.length?status.results:[{id:'gbrain',status:'not_checked',version:status.gbrain_version},{id:'skills',status:'not_checked'}]).filter(r=>r.status!=='not_adopted');
