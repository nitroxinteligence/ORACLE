...
atlasController.setDepartment('department/code',{immediate:true});await wait(50);check('department drilldown preserves 118 percent',fixed()&&atlasController.department==='department/code');
...
ob.status='completed';ob.message='Concluído';await OracleOnboarding.poll();check('native completion closes onboarding directly to the graph without a tour',document.querySelectorAll('dialog[open]').length===0);
...
document.querySelector('.ob-close').click();await wait(80);
