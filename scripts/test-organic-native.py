...
call('selected=null;renderAtlas();atlasController.fit();true');settle();start_width=call('atlasController.viewport.width')
call('toggleObservatory(true);true');settle()
check('Observatory reduces actual graph viewport',f'atlasController.viewport.width<{start_width}-100 && !$("#observatory-panel").hidden && $("#observatory-toggle").getAttribute("aria-expanded")==="true"')
call('toggleObservatory(false);true');settle()
check('minimize restores graph area',f'Math.abs(atlasController.viewport.width-{start_width})<1 && $("#observatory-panel").hidden')
