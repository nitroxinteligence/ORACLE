...
class OracleAtlas {
  constructor(element, callbacks) {
    ...
    <g class="oracle-core" role="button" tabindex="0" data-core="true" aria-label="Oracle, núcleo central. Enquadrar todo o atlas">
      ...
      <circle r="55" fill="transparent" class="core-hit"/><text class="core-word" y="13">ORACLE</text><text class="core-caption" y="27">MEMÓRIA · CONTEXTO</text>
    </g></g></svg><div class="atlas-caption"><span class="atlas-coordinate">ATLAS / 01</span><span class="atlas-instruction">Arraste para explorar · scroll para aproximar</span></div><div class="atlas-selection-hint" hidden></div>`;
  ...
  resize(){const r=this.el.getBoundingClientRect();this.width=r.width;this.height=r.height;if(!this.width||!this.height)return;this.contentTop=145;this.baseScale=Math.max(.3,Math.min((this.width-58)/880,(this.height-this.contentTop-42)/650));if(this.first){this.camera={x:this.width/2,y:(this.height+this.contentTop)/2-20*this.baseScale,k:this.baseScale};this.target={...this.camera};this.first=false}else{this.target.x+=this.width/2-(this.lastWidth||this.width)/2;this.target.y+=this.height/2-(this.lastHeight||this.height)/2}this.lastWidth=this.width;this.lastHeight=this.height;this.invalidate()}
  update(data){this.data=data;this.selected=data.selected;this.selectedLeaf=data.selectedLeaf||null;this.reduced=!!data.reduced||!!this.motionPreference?.matches;this.el.classList.toggle('motion-reduced',!!this.reduced);this.setPaused(document.hidden||data.hidden||data.paused);
    if(!this.loadedLayout&&data.layout){this.layout={nodes:data.layout.nodes||{},leaves:data.layout.leaves||{}};this.loadedLayout=true}
    ...
    collections.forEach((c,i)=>{let node=this.nodes.get(c.id);if(!node){const p=this.layout.nodes[c.id]||{x:this.defaults[i][0],y:this.defaults[i][1]};
      ...
      node.skills=(data.entries||[]).filter(e=>!e.directory&&e.path.startsWith(`SISTEMA/skills/${c.id}/`)&&e.name==='SKILL.md');node.count.textContent=node.skills.length?`${node.skills.length} skills`:'Sem skills nesta fonte';node.g.setAttribute('aria-label',`${c.name}, ${node.skills.length} skills. Enter seleciona; espaço aproxima; setas movem o layout.`);
    });
    this.lastSelectionKey=null;this.buildLeaves();this.selection();this.draw();
    if(data.events?.length){const last=data.events.at(-1);this.universe?.signalReceipt(last)}
  }
  buildLeaves(){if(!this.data)return;const level=this.camera.k/this.baseScale;const ids=[];for(const n of this.nodes.values()){const focused=this.selected===n.id;let count=focused?Math.min(8,3+Number(this.data.detail||0)):level>1.45?3:2;count=Math.min(count,n.skills.length);for(let i=0;i<count;i++){const e=n.skills[i];ids.push(e.path);if(this.leaves.has(e.path))continue;const dir=n.x>=0?1:-1;const dy=(i-(Math.min(8,n.skills.length)-1)/2)*32;const saved=this.layout.leaves[e.path];const p=saved||{x:n.x+dir*(106+(i%2)*12),y:n.y+dy};
      ...
      g.setAttribute('aria-label',`${name}. Enter seleciona; espaço abre documento; setas movem o layout.`);
      this.lastSelectionKey=null;this.leaves.set(e.path,{id:e.path,parent:n.id,x:p.x,y:p.y,edge,g,label,custom:!!saved,dir,index:i});
    }}
    for(const [id,leaf]of this.leaves){if(!ids.includes(id)){leaf.g.remove();leaf.edge.remove();this.leaves.delete(id);this.lastSelectionKey=null}}
  }
  selection(){const key=[this.selected,this.selectedLeaf,this.camera.k/this.baseScale>1.35,this.leaves.size,this.nodes.size].join('|');if(key===this.lastSelectionKey)return;this.lastSelectionKey=key;for(const node of this.nodes.values()){const active=node.id===this.selected;node.g.classList.toggle('selected',active);node.g.classList.toggle('subdued',!!this.selected&&!active);node.edge.classList.toggle('selected',active);node.edge.classList.toggle('subdued',!!this.selected&&!active);node.flow.classList.toggle('flow-visible',active||(!this.selected&&node.index===1));node.flow.classList.toggle('selected',active);node.g.setAttribute('aria-pressed',String(active))}
  ...
  draw(){if(this.disposed)return;const started=performance.now();const {x,y,k}=this.camera;
    this.world.setAttribute('transform',`translate(${x} ${y}) scale(${k})`);const labelK=Math.max(.7,Math.min(1.15,this.baseScale/k));
    const scaleChanged=labelK!==this.lastLabelK;
    for(const n of this.nodes.values()){
      if(n._x!==n.x||n._y!==n.y){n.g.setAttribute('transform',`translate(${n.x} ${n.y})`);const path=this.edgePath(0,0,n.x,n.y);n.edge.setAttribute('d',path);n.flow.setAttribute('d',path);n._x=n.x;n._y=n.y}
      if(scaleChanged||!n._labelSized){n.label.style.fontSize=`${18*labelK}px`;n.count.style.fontSize=`${12*labelK}px`;n._labelSized=true}
    }
    for(const leaf of this.leaves.values()){const p=this.nodes.get(leaf.parent);
      if(leaf._x!==leaf.x||leaf._y!==leaf.y||leaf._px!==p.x||leaf._py!==p.y){leaf.g.setAttribute('transform',`translate(${leaf.x} ${leaf.y})`);leaf.edge.setAttribute('d',this.edgePath(p.x,p.y,leaf.x,leaf.y,.06));leaf._x=leaf.x;leaf._y=leaf.y;leaf._px=p.x;leaf._py=p.y}
      if(scaleChanged||!leaf._labelSized){leaf.label.style.fontSize=`${13*labelK}px`;leaf._labelSized=true}
    }
    this.lastLabelK=labelK;if(this.lastZoom!==k){this.cb.onZoom?.(k/this.baseScale);this.lastZoom=k}
    this.selection();this.universe?.sync(this);this.updateCosts.push(performance.now()-started);if(this.updateCosts.length>180)this.updateCosts.shift();
  }
  invalidate(){if(this.disposed||this.frame||document.hidden||window.oracleWindowVisible===false||this.data?.hidden)return;this.frame=requestAnimationFrame(()=>this.tick())}
  tick(){const now=performance.now();if(this.lastTick&&now-this.lastTick<90){this.interactionFrames.push(now-this.lastTick);if(this.interactionFrames.length>180)this.interactionFrames.shift()}this.lastTick=now;this.frame=0;const d=Math.max(Math.abs(this.target.x-this.camera.x),Math.abs(this.target.y-this.camera.y),Math.abs(this.target.k-this.camera.k)*100);if(d>.02){const t=this.reduced?1:.26;for(const key of ['x','y','k'])this.camera[key]+=(this.target[key]-this.camera[key])*t;this.draw();this.invalidate()}else{this.camera={...this.target};this.buildLeaves();this.draw();this.selection()}}
  ...
  focus(id){this.finishFormationForInput();const n=this.nodes.get(id);if(!n)return;this.previous={...this.target};const k=this.baseScale*1.75;this.target={x:this.width/2-n.x*k-(n.x>0?50:-50),y:(this.height+this.contentTop)/2-n.y*k,k};this.invalidate()}
  select(category,leaf=null,keyboard=false){this.finishFormationForInput();this.selected=category;this.selectedLeaf=leaf;this.buildLeaves();this.selection();this.draw();this.cb.onSelect?.(category,leaf,keyboard)}
  ...
  translateNode(node,dx,dy,category){const x=Math.max(-1500,Math.min(1500,node.x+dx)),y=Math.max(-1200,Math.min(1200,node.y+dy));dx=x-node.x;dy=y-node.y;node.x=x;node.y=y;if(category){for(const leaf of this.leaves.values()){if(leaf.parent===category&&!leaf.custom){leaf.x+=dx;leaf.y+=dy}}}else node.custom=true}
  persistSoon(){clearTimeout(this.saveTimer);this.saveTimer=setTimeout(()=>{this.cb.onLayout?.(this.layout)},300)}
  ...
  finishFormationForInput(){if(this.getFormation().progress<1)this.setFormation({progress:1,playing:false})}
  ...
  bind(){
    ...
    this.listen(this.el,'keydown',e=>{const cat=e.target.closest('[data-category]')?.dataset.category,skill=e.target.closest('[data-skill]')?.dataset.skill;const node=skill?this.leaves.get(skill):cat?this.nodes.get(cat):null;
