...
func runNativeMemoryEngineTests() throws {
    ...
    try writeJSON(["owner":"OracleCompanion","schema_version":1],profile.appendingPathComponent("oracle-owned.json"))
    core.config["gbrainAccess"]=true;core.config["gbrainVaultSource"]="oracle-vault";try core.persist()
    func write(_ path:String,_ text:String)throws { try Data(text.utf8).write(to:root.appendingPathComponent(path),options:.atomic) }
    func expect(_ value:Bool,_ message:String)throws { guard value else { throw failure(message) };print("PASS \(message)") }
    ...
    try Data("{\"synthetic\":true}".utf8).write(to:obsidian.appendingPathComponent("workspace.json"),options:.atomic)
