...
export async function indexVault(engine:BrainEngine,input:any){
 ...
 const event=(type:string,completed:number,summary:string)=>{const id=sha(run+type+completed);atomic(join(receiptDir,id+'.json'),{schema_version:1,event_id:id,source:'gbrain',event_type:type,phase:'index',completed,total:files.length,run_id:run,received_at:now(),occurred_at:now(),sanitized_summary:summary,coverage:'derived-vault-source',observed_status:type.endsWith('failed')?'failed':'verified'})};
