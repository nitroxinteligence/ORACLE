...
**Integração de 11/09/2026:** onboarding e auditoria completa conciliados no mesmo código. A entrega local 0.3.0 é um pré-lançamento macOS arm64 com assinatura ad hoc, não uma distribuição notarizada pela Apple. Validação: 481 verificações nativas, 51 testes de navegação/leitor e 48 verificações em WKWebView; o protocolo MCP real passou em 68 verificações com GBrain/PGLite. Escopos e limitações estão no [registro da integração](docs/implementation/integrated-delivery-20260911.md).

## Usar

1. Gere o pedido direto deste Mac (`ORACLE-MAC2`) ou use seu convite individual `ORACLEINV2` para gerar uma solicitação `ORACLEREQ2`. Receba do proprietário a resposta assinada `ORACLE2` correspondente e importe-a. O app não precisa consultar um servidor; o convite com prova de chave exige Secure Enclave disponível.
