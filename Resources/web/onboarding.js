...
function render(){
 ...
 if(stage==='license'){
  ...
  frame('Conecte ao Codex.',`<p>O Codex executa a instalação. O Oracle acompanha cada etapa e mantém seu conhecimento no Obsidian.</p><div class="ob-status">${current.codexConnected?'Codex conectado':'Autorize sua conta ChatGPT no Codex para continuar.'}</div><p class="ob-muted">A autorização acontece na página do Codex. Senhas e tokens ficam sob controle dele.</p>`,button('ob-connect',current.codexConnected?'Verificar conexão':'Conectar Codex',!current.codexConnected)+(current.authorizing?button('ob-cancel-login','Cancelar autorização',false):'')+button('ob-next','Continuar',true));
  ...
  frame(current.status==='completed'?'Seu Oracle está pronto.':'Seu universo está tomando forma.',`<p>${esc(current.message||'Continue a configuração do seu Oracle.')}</p><ul class="ob-confirmed">${(current.confirmed||[]).filter(x=>x.kind!=='skill').map(x=>`<li>✓ ${esc(x.label)}</li>`).join('')}</ul>${(current.confirmed||[]).some(x=>x.kind==='skill')?`<p>${current.confirmed.filter(x=>x.kind==='skill').length} skills verificadas</p>`:''}${current.detail?`<details><summary>Ver detalhes</summary><p>${esc(current.detail)}</p></details>`:''}${current.status==='completed'?'<p class="ob-muted">A observação de tarefas por hooks pode ser ativada pelo mecanismo de confiança do Codex.</p>':''}`,button('ob-progress-close','Voltar ao universo',false)+button('ob-open-codex','Abrir no Codex',false)+(!active.has(current.status)&&current.status!=='completed'?button('ob-resume','Retomar'):''));
