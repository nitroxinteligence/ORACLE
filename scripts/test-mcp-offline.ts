...
const adapter=join(fixture,'engine/oracle-gbrain-read'),profile=join(fixture,'profile'),vault=join(fixture,'vault');
...
const env={PATH:'/usr/bin:/bin:/usr/sbin:/sbin',HOME:join(fixture,'home'),TMPDIR:join(fixture,'tmp'),
 ...
 ORACLE_RECEIPT_DIR:join(fixture,'events'),DO_NOT_TRACK:'1'};
...
try{
 ...
 await section('limits',async()=>{
  ...
  try{
   const disabled=await denied('put_page',{slug:'disabled-policy-write',content:'# Never persist'},/keyword-only search enabled in the selected database/);
