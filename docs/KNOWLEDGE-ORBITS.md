# Órbitas pessoal e profissional
...
Os novos planos incluem `knowledge_spaces` e a criação das áreas que faltam, mesmo quando o usuário aproveita seu vault. Os destinos padrão são `AREAS/pessoal` e `AREAS/profissional`; pastas equivalentes já existentes (`Pessoal`, `Profissional`, inclusive variações de caixa) são reaproveitadas. A revisão do onboarding informa essa organização antes da confirmação. Planos já confirmados não são alterados.

As skills distribuídas orientam novas notas duráveis autorizadas para a área apropriada, preservando origem e caminhos. Notas existentes não são movidas ou classificadas automaticamente. A captura literal em `INBOX/oracle-memory` conserva seu contrato; o índice GBrain do vault continua derivado dos arquivos canônicos.

Áreas ainda inexistentes são identificadas como tal no tooltip/estado vazio. A visualização não inventa notas nem representa indexação ou gravação sem recibo.

## Verificação

- 18 verificações de geometria, hierarquia e limites (6 específicas de conhecimento).
- 23 contratos nativos, incluindo reaproveitamento de Pessoal, criação de Profissional, ausência de duplicatas e preservação de notas.
- 28 verificações de onboarding + 16 do ciclo de execução, com fixtures.
- [15 verificações no AppKit/WKWebView](evidence/skilltree/knowledge-native.json), incluindo 103 notas em páginas 50/50/3, leitor real, retorno, tooltip, ícone padrão e exclusão dos três conectores da órbita.
