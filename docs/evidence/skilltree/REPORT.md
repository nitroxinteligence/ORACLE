...
Instalação principal: `/Applications/Oracle.app` atualizada e aberta, com 4.934 arquivos conferidos contra o bundle local e configuração preservada. Duas cópias indexadas foram movidas para backup oculto; 13 registros de bundles de desenvolvimento/QA foram retirados do Launch Services. Spotlight e Launch Services retornam apenas o Oracle principal. Compilações futuras usam `.work/build/Oracle.app`; DMG permanece em `dist`. Integração fast-forward enviada para `origin/main`, sem sobrescrever histórico remoto. [Recibo](installed-macos.json).

Nomes de especialistas em título H3 de 20 px, sem tooltip. Hover destaca o especialista e seus ramos; demais especialistas e elementos periféricos ficam suavemente esmaecidos/desfocados. Ao sair, a visualização normal retorna. Validação encerrada a pedido do usuário; prosseguindo somente com commit, push e instalação.
