    ...
    func userContentController(_ userContentController:WKUserContentController,didReceive message:WKScriptMessage){
        ...
        if method=="fixtureSnapshot" {
            ...
            web.takeSnapshot(with:nil) {image,error in
                ...
                var saved=false
                if let tiff=image?.tiffRepresentation,let bitmap=NSBitmapImageRep(data:tiff),let png=bitmap.representation(using:.png,properties:[:]) {do{try png.write(to:destination);saved=true}catch{}}
