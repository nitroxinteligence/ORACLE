...
class OracleUniverse {
  ...
  sync(model) {
    if (!this.active) return;
    this.model = model;
    const showSun=!model.selected&&!model.department&&!model.knowledge;
    ...
    const values = {
      ...
      uFocusedCluster: this.clusterRows.get(model.context?.group) ?? -2, uHoveredCluster: this.clusterRows.get(hover.group || model.leaves.get(hover.skill)?.group) ?? -2, uContext: ['global','specialist','group','skill','knowledge','department'].indexOf(model.context?.kind),
    };
    ...
    for (const n of model.nodes.values()) {
      ...
      const selected = n.id === model.selected || model.knowledge || n.kind==='department'&&n.id===model.department&&!model.selected ? 1 : 0;
  ...
  syncGeometry(model) {
    ...
    for (const n of model.nodes.values()) {
      ...
      const parent=model.nodes.get(n.parent);
      if(parent)edgeTo(parent.x,parent.y,n.x,n.y,color,0,n.index,-1,-3);
      else if(!model.selected&&!model.department&&!model.knowledge)edgeTo(0, 0, n.x, n.y, color, 0, n.index, -1, -3);
      row++;
    }
    let clusterRow = 0;
