...
extension Core {
    ...
    func engineEnvironment(existing:Bool=false)->[String:String] {
        ...
        if !existing {
            ...
            env["GBRAIN_HOME"]=config["gbrainProfile"] as? String ?? config["gbrainWorkspace"] as? String ?? home.appendingPathComponent("gbrain/unselected-profile").path
        }
        return env
    }
    ...
    func indexVaultSnapshot(_ snapshot:VaultScanSnapshot,generation:Int,planRef:String? = nil,
                           budget:Double = 25,maxUpserts:Int = 500,cancellation:URL? = nil,
                           force:Bool = false) throws -> [String:Any] {
        try withVaultWrite {
            let root=try vault().resolvingSymlinksInPath().standardizedFileURL
            guard root==snapshot.root,config["gbrainWorkspace"]==nil else{throw failure("A seleção da memória mudou.")}
            ...
            guard owner["owner"] as? String=="OracleCompanion",owner["schema_version"] as? Int==2,
                  owner["vault_root"] as? String==root.path else{throw failure("Fonte e destino do índice precisam ser confirmados novamente.")}
            try prepareEngineScratch();try prepareOwnedGBrainRuntime()
            var env=engineEnvironment();env["ORACLE_RECEIPT_DIR"]=try scoped("events",root:home).path
            if let planRef {
                guard UUID(uuidString:planRef) != nil else{throw failure("Identificador do plano inválido.")}
                env["ORACLE_PLAN_EVENTS_DIR"]=try scoped("setup/events/"+planRef,root:home).path
            ...
            do {
                ...
                if value["complete"] as? Bool==true {
                    ...
                    value["status"]="verified";value["checkedAt"]=ISO8601DateFormatter().string(from:Date());value["inference"]=false
                    try writeJSON(value,receipt)
                } else {
                    value["status"]="partial"
                    var last=(try? readJSON(receipt)) ?? [:];last["status"]="needs_attention";last["lastAttempt"]=value
                    try writeJSON(last,receipt)
                }
                return value
            } catch {
                var last=(try? readJSON(receipt)) ?? [:];last["status"]="needs_attention";last["attemptedAt"]=ISO8601DateFormatter().string(from:Date())
